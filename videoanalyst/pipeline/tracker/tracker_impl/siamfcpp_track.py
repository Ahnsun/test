...
class SiamFCppTracker(PipelineBase):
    ...
    def init(self, im, state):
        ...
        :return: None
        """
        rect = state  # bbox in xywh format is given for initialization in case of tracking（这里可以尝试将多个目标的模板的bbox进行设置）
        box = xywh2cxywh(rect) #将顶点坐标转换成中心坐标
        target_pos, target_sz = box[:2], box[2:] #获取目标位置和目标框尺寸

        self._state['im_h'] = im.shape[0] #图片的高
        self._state['im_w'] = im.shape[1] #图片的宽

        # extract template feature（这里可以尝试获取多个目标的特征）
    ...
    def track(self,
        ...
        # score post-processing（从得分图中得到获取分数最高的id）
    ...
    def _update_state(self, mstate, key_a, key_b, val):
        if key_a in mstate:
            mstate[key_a].update({key_b: val})
        else:
            mstate.update({key_a: {key_b: val}})
...
####################################################################################################################
    ...
    def mot_init(self, im, tracks):
        r"""
        Initialize mot_tracker
            Internal target state representation: self._state['m_state'] = ((target1_pos, target1_sz),(target2_pos, target2_sz),......(targetn_pos,targetn_size))
        :param im: initial frame image
        :param state: bbox, format: xywh
        :return: None
        """
        ...
        for track in tracks:
            id = track['id']
            rect = track['box']  # bbox in xywh format is given for initialization in case of tracking（这里可以尝试将多个目标的模板的bbox进行设置）
            box = xywh2cxywh(rect)  # 将顶点坐标变换成中心点坐标
            target_pos, target_sz = box[:2], box[2:]  # 获取目标位置和目标框尺寸
            ...
            self._update_state(self._mstate, id, 'z_crop', im_z_crop)
            self._update_state(self._mstate, id, 'avg_chans', avg_chans)
            self._update_state(self._mstate, id, 'features', features)
            self._update_state(self._mstate, id, 'window', window)
            self._update_state(self._mstate, id, 'state', (target_pos, target_sz))
    ...
    def mot_track(self,
        ...
        if 'avg_chans' in kwargs:
            ...
            avg_chans = self._mstate[id]['avg_chans']
        ...
        # score post-processing（得到获取分数最高的id）
        best_pscore_id, pscore, penalty = self._mot_postprocess_score(
            id, score, box_wh, target_sz, scale_x)
        # box post-processing
        new_target_pos, new_target_sz = self._postprocess_box(
            best_pscore_id, score, box_wh, target_pos, target_sz, scale_x,
            x_size, penalty)
        ...
        # record basic mid-level info
        self._mstate[id]['x_crop'] = im_x_crop
        bbox_pred_in_crop = np.rint(box[best_pscore_id]).astype(np.int)
        self._mstate[id]['bbox_pred_in_crop'] = bbox_pred_in_crop
        # record optional mid-level info
        if update_state:
            self._mstate[id]['score'] = score
            self._mstate[id]['pscore'] = pscore
            self._mstate[id]['all_box'] = box
            self._mstate[id]['cls'] = cls
            self._mstate[id]['ctr'] = ctr

        return new_target_pos, new_target_sz


    def mot_update(self, im, tracks):
        ...
        for track in tracks:
            ...
            id = track['id']
            pause = track['pause']
            box = xywh2cxywh(track['box'])  # 将顶点坐标变换成中心点坐标
            target_pos_prior, target_sz_prior = box[:2], box[2:]
            features = self._mstate[id]['features']

            # forward inference to estimate new state
            target_pos, target_sz = self.mot_track(id,
                                                   im,
                                               target_pos_prior,
                                               target_sz_prior,
                                               features,
            ...
            # save underlying state
            # self.state['target_pos'], self.state['target_sz'] = target_pos, target_sz
            self._mstate[id]['state'] = target_pos, target_sz

            # return rect format
            track_rect = cxywh2xywh(np.concatenate([target_pos, target_sz],
                                                   axis=-1))
            track_rects.append({'id':id,'pause':pause,'box':track_rect})
            track_boxes.append(track_rect)

        return track_rects, np.array(track_boxes)


    def _mot_postprocess_score(self, id, score, box_wh, target_sz, scale_x):
        r"""
        Perform SiameseRPN-based tracker's post-processing of score
        :param score: (HW, ), score prediction
        :param box_wh: (HW, 4), cxywh, bbox prediction (format changed)
        ...
        window_influence = self._hyper_params['window_influence']
        pscore = pscore * (
            1 - window_influence) + self._mstate[id]['window'] * window_influence
        best_pscore_id = np.argmax(pscore)

        return best_pscore_id, pscore, penalty

    def add_init(self, im, tracks):
        r'''
        When new target appear in the video,we need to add new feature and target
        :param im:
        :param m_state:
        :return:
        '''
